...
public class JwtFilter extends OncePerRequestFilter {
	...
	protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
		...
		return uri.startsWith("/oauth2/") || uri.startsWith("/login/") || uri.equals("/oauth-callback")
				|| uri.startsWith("/api/auth/") || uri.equals("/api/members/login");
	}
		...
		if (authorization != null && authorization.startsWith("Bearer ")) {
			...
			try {
				...
				if (memberId.startsWith("#") || memberId.matches("^[0-9]+$")) {
					...
					CustomUserDetails socialUser = CustomUserDetails.builder().username(memberId)
							.memberName("Social User") // 필요시 claims에서 닉네임 추출 가능
							.authorities(Collections.singleton(new SimpleGrantedAuthority(role))).build();

					UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
							socialUser, // Principal에 객체 전달
							null, socialUser.getAuthorities());
